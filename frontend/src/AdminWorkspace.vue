...
type MainTab = 'files' | 'admin'

const authMode = ref<AuthMode>('login')
const mainTab = ref<MainTab>('files')
...
const adminUserFiles = ref<FileItem[]>([])
...
const adminStats = computed(() => {
  const totalSize = adminUsers.value.reduce((sum, item) => sum + item.totalSize, 0)
  return {
...
async function loadAdminUsers() {
  ...
  try {
    ...
    if (!selectedAdminUserId.value && data.items.length > 0) {
      selectedAdminUserId.value = data.items[0].id
    }
    if (selectedAdminUserId.value) {
      await loadAdminUserFiles(selectedAdminUserId.value)
    } else {
      adminUserFiles.value = []
...
async function loadAdminUserFiles(userId: string) {
  if (!userId) {
    adminUserFiles.value = []
    adminUser.value = null
    return
  ...
  pageLoading.value = true
  try {
    const data = await listAdminUserFiles(userId)
    adminUser.value = data.user
    adminUserFiles.value = data.items
  } finally {
    pageLoading.value = false
  }
}
...
async function handleRename(item: FileItem) {
  try {
    ...
    if (currentUser.value?.isAdmin && mainTab.value === 'admin' && selectedAdminUserId.value) {
      await loadAdminUserFiles(selectedAdminUserId.value)
      await loadAdminUsers()
    } else {
      await loadMyFiles()
...
async function handleDeleteAdminUser(user: AdminUser) {
  try {
    await ElMessageBox.confirm(`确认删除用户 "${user.username}" 及其所有文件吗？`, '删除用户', {
      confirmButtonText: '删除',
      cancelButtonText: '取消',
      type: 'warning'
    })
    ...
    if (selectedAdminUserId.value === user.id) {
      selectedAdminUserId.value = ''
      adminUserFiles.value = []
      adminUser.value = null
    }
...
async function handleAdminFileDelete(item: FileItem) {
  try {
    await ElMessageBox.confirm(`确认删除文件 "${item.originalName}" 吗？`, '删除文件', {
      confirmButtonText: '删除',
      cancelButtonText: '取消',
...
function selectAdminUser(userId: string) {
  ...
  void loadAdminUserFiles(userId)
}

function userRowClass(row: AdminUser) {
  return row.id === selectedAdminUserId.value ? 'is-selected-user' : ''
}
  ...
  <div class="app-shell">
    ...
    <main class="workspace-shell">
      ...
      <section class="panel content-panel">
        ...
        <template v-else>
          <el-tabs v-model="mainTab" class="auth-tabs">
            <el-tab-pane label="我的文件" name="files" />
            <el-tab-pane v-if="currentUser.isAdmin" label="后台管理" name="admin" />
          </el-tabs>

          <template v-if="mainTab === 'files'">
            <div class="panel-header">
              <div>
          ...
          <template v-else>
            <div class="admin-shell">
              <div class="panel-header">
                <div>
                  <h2>后台管理</h2>
                  <p class="subtle">管理用户及其归属文件。</p>
                </div>
                <div class="admin-summary">
                  <div class="admin-summary-item">
                    <span>用户总数</span>
                    <strong>{{ adminStats.userCount }}</strong>
                  </div>
                  <div class="admin-summary-item">
                    <span>全站文件大小</span>
                    <strong>{{ formatBytes(adminStats.totalSize) }}</strong>
                  </div>
                </div>
              </div>
              ...
              <div class="admin-layout">
                <aside class="admin-sidebar">
                  <div class="card-title-row">
                    <h3>用户列表</h3>
                    <el-button text @click="loadAdminUsers">刷新</el-button>
                  </div>
                  ...
                  <el-table
                    ...
                    @row-click="(row: AdminUser) => selectAdminUser(row.id)"
                  >
                  ...
                  <div class="sidebar-footer" v-if="selectedAdminUserId">
                    <el-button
                      text
                      type="danger"
                      class="sidebar-delete"
                      @click="handleDeleteAdminUser(adminUsers.find((user) => user.id === selectedAdminUserId)!)"
                    >
                      删除当前用户
                    </el-button>
                  </div>
                ...
                <section class="admin-content">
                  <div class="card-title-row">
                    <h3>{{ adminUser ? `${adminUser.username} 的文件` : '请选择一个用户' }}</h3>
                    <span class="subtle">{{ adminUserFiles.length }} 个文件</span>
                  </div>

                  <el-table :data="adminUserFiles" stripe class="files-table" v-loading="pageLoading">
                    <el-table-column prop="originalName" label="文件名" min-width="180" />
                    <el-table-column prop="sizeBytes" label="大小" width="120">
                      <template #default="{ row }">
                        {{ formatBytes(row.sizeBytes) }}
                    ...
                    <el-table-column label="操作" width="120" fixed="right">
                      <template #default="{ row }">
                        <el-button link type="primary" @click="handleDownload(row)">下载</el-button>
                        <el-button link type="danger" @click="handleAdminFileDelete(row)">删除</el-button>
                      </template>
                    </el-table-column>
                  </el-table>

                  <el-empty
                    v-if="!pageLoading && selectedAdminUserId && adminUserFiles.length === 0"
                    description="该用户暂无文件"
                  />
                </section>
              </div>
